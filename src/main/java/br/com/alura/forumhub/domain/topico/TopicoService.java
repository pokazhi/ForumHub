...
public class TopicoService {
    ...
    @Autowired
    private com.iaco.forumhub.domain.topico.TopicoRepository topicoRepository;
    ...
    public Curso getCurso(com.iaco.forumhub.domain.topico.DadosNovoTopico dadosNovoTopico) {
    ...
    public com.iaco.forumhub.domain.topico.DadosDetalhamentoTopico getDadosDetalhamentoTopicoAtualizado(Long id, com.iaco.forumhub.domain.topico.DadosAtualizarTopico dadosAtualizarTopico, Authentication authentication) {
        var usuario = authenticationService.getUsuario(authentication);
        Optional<com.iaco.forumhub.domain.topico.Topico> topico = topicoRepository.findById(id);
        if (topico.isEmpty()) {
        ...
        var dadosTopicoAtualizado = new com.iaco.forumhub.domain.topico.DadosDetalhamentoTopico(topico.get());
        return dadosTopicoAtualizado;
    }

    public com.iaco.forumhub.domain.topico.DadosDetalhamentoTopico getDadosDetalhamentoTopicoDeletado(Long id, Authentication authentication) {
        var usuario = authenticationService.getUsuario(authentication);
        Optional<com.iaco.forumhub.domain.topico.Topico> topico = topicoRepository.findById(id);
        if (topico.isEmpty()) {
            throw new ValidacaoException("Tópico não encontrado");
        ...
        topicoRepository.save(topico.get());
        var dadosTopicoDeletado = new com.iaco.forumhub.domain.topico.DadosDetalhamentoTopico(topico.get());
        return dadosTopicoDeletado;
    }

    public Optional<com.iaco.forumhub.domain.topico.Topico> getTopico(DadosNovaResposta dadosNovaResposta) {
